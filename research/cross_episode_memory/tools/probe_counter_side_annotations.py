...
m,d,_=make_kitchen(assets,RBY1MConfig(),(F,EGG.rsplit('_1_0_0',1)[0],POTATO.rsplit('_1_0_0',1)[0]),robot_xy=(.01,1.78));d.qpos[:]=rows[0]['qpos'];mujoco.mj_forward(m,d)
...
for i,pose in enumerate(world):
 ...
 if direction[0]<-.7 and abs(pose[2,1])<.3 and abs(direction[2])<.3:out.append((float(width),i,direction.tolist()))
